...
def convert_patterns(appInstance, patterns):
    results = []
    ...
    if patterns[1] != "":  # Second search field non-empty
        if appInstance.and_or_state.get() == 1:  # if OR, just make
...
def make_list(appInstance, search_results):
    local_output_list = []
    ...
    for item in search_results:
        ...
        if item[0] is None:
            item[0] = ROOT
        if appInstance.search_root not in item[0]:
            continue
...
def searcher_submit(appInstance, search_term1, search_term2, operation):
    output_results = []
    ...
    if search_term1 != "":  # check to see if anything was entered
        ...
        appInstance.results_window.config(state=tkt.NORMAL)
        appInstance.results_window.tag_config("file", foreground="blue")

        search_terms = [search_term1, search_term2]

        # make search_terms into regex patterns
        outer_patterns = convert_patterns(appInstance, search_terms)
        ...
        output_final = make_list(appInstance, output_results)
        send_to_window(appInstance, output_final)

    appInstance.line_index += len(output_final)
    appInstance.filecount.config(text="Files Found: %d" % (
                                 appInstance.line_index - 1), fg="black")
    appInstance.cleared = False

    # DISABLES EDITING IN RESULT WINDOW
    appInstance.results_window.config(state=tkt.DISABLED)


def send_to_window(appInstance, output):

    for ind_x, line in enumerate(output):
        appInstance.results_window.insert(tkt.END, line)
        #  _____ crazy way of highlighting file names in results window
        filename_end = str(ind_x +
                           appInstance.line_index) + "." + str(len(line))

        filename_begin = str(ind_x + appInstance.line_index) + "." +\
            str((int(filename_end.split(".")[-1]) - len(line.split("/")[-1])))

        appInstance.results_window.tag_add("file", filename_begin,
                                           filename_end)

    appInstance.results_window.see(tkt.END)
...
class MyApp:
    ...
    def search_funct(self, event=None):
        ...
        # for testing
        # print self.endings
        searcher_submit(self, self.ent.get(), self.ent2.get(),
